...
function ProgressStyles() {
  return <style>{`
    .progress-dashboard{display:grid;gap:18px}
    .progress-hero{background:linear-gradient(145deg,#101827,#18243a);color:#fff;border-radius:22px;padding:24px;display:flex;align-items:center;justify-content:space-between;gap:24px}
    .progress-hero h2{margin:0 0 6px;font-size:22px}.progress-hero p{margin:0;color:#aab4c7;font-size:12px;line-height:1.6}
    .progress-score{min-width:112px;height:112px;border-radius:50%;display:grid;place-items:center;background:conic-gradient(#7c6cff 0deg,#7c6cff 0deg,#2b3548 0deg);position:relative}
    .progress-score:after{content:"";position:absolute;inset:9px;border-radius:50%;background:#101827}
    .progress-score strong,.progress-score span{position:relative;z-index:1}.progress-score strong{font-size:25px}.progress-score span{font-size:9px;color:#aab4c7;display:block;text-align:center}
    .progress-metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}
    .progress-metric{background:#fff;border:1px solid #e7e8ef;border-radius:16px;padding:17px}.progress-metric strong{display:block;font-size:23px}.progress-metric span{display:block;margin-top:4px;color:#7d8291;font-size:10px}
    .progress-panel{background:#fff;border:1px solid #e7e8ef;border-radius:18px;padding:20px}.progress-panel h2{margin:0;font-size:15px}.progress-panel p{margin:5px 0 16px;color:#858a99;font-size:10px}
    .progress-row{margin-top:14px}.progress-row-head{display:flex;justify-content:space-between;font-size:10px;font-weight:800;margin-bottom:6px}.progress-row-head span:last-child{color:#6d5dfc}
    .progress-track{height:7px;background:#ececf2;border-radius:99px;overflow:hidden}.progress-track div{height:100%;border-radius:99px;background:linear-gradient(90deg,#6d5dfc,#8b7cff)}
    @media(max-width:760px){.progress-hero{align-items:flex-start}.progress-score{min-width:88px;width:88px;height:88px}.progress-score strong{font-size:20px}.progress-metrics{grid-template-columns:repeat(2,1fr)}}
  `}</style>;
}
...
function HomeStyles() {
  return <style>{`
    ...
    .home-welcome{background:linear-gradient(135deg,#242847 0%,#34345f 100%);color:#fff;border-radius:22px;padding:28px 30px;display:flex;align-items:center;justify-content:space-between;gap:24px;overflow:hidden;position:relative}
    .home-welcome:after{content:"";position:absolute;width:260px;height:260px;border:1px solid rgba(255,255,255,.08);border-radius:50%;right:-80px;top:-120px;box-shadow:0 0 0 55px rgba(255,255,255,.025),0 0 0 110px rgba(255,255,255,.018)}
    .home-welcome-copy{position:relative;z-index:1;max-width:680px}
    .home-welcome .eyebrow{color:#b8adff;margin-bottom:10px}
    .home-welcome h1{font-size:32px;line-height:1.12;margin:0 0 8px;color:#fff}
    .home-welcome p{margin:0;color:#d7d9ed;line-height:1.6;font-size:14px}
    .home-welcome-action{position:relative;z-index:1;flex:0 0 auto}
    .home-primary{border:0;border-radius:12px;background:#6c5ce7;color:#fff;padding:13px 18px;font-weight:800;display:inline-flex;align-items:center;gap:8px;cursor:pointer;box-shadow:0 10px 24px rgba(108,92,231,.28)}
    .home-primary:hover{filter:brightness(1.06)}
    .home-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}
    .home-stat{background:#fff;border:1px solid #e6e8f0;border-radius:16px;padding:18px;display:flex;align-items:center;gap:14px;min-width:0}
    .home-stat-icon{width:42px;height:42px;border-radius:12px;display:grid;place-items:center;background:#eef0ff;color:#6758e8;flex:0 0 auto}
    .home-stat strong{display:block;font-size:24px;line-height:1.1;color:#111827}
    .home-stat span{display:block;margin-top:4px;font-size:12px;color:#737b91}
    .home-section{display:grid;gap:12px}
    .home-section-head{display:flex;align-items:end;justify-content:space-between;gap:16px}
    .home-section-head h2{margin:0;font-size:21px}
    .home-section-head p{margin:4px 0 0;color:#7b8398;font-size:13px}
    .home-link{border:0;background:transparent;color:#6657e8;font-weight:800;cursor:pointer;display:inline-flex;align-items:center;gap:3px}
    .home-decks{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}
    .home-deck{background:#fff;border:1px solid #e3e6ef;border-radius:16px;padding:18px;transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease}
    .home-deck:hover{transform:translateY(-2px);box-shadow:0 10px 28px rgba(31,38,67,.08);border-color:#d6d2ff}
    .home-deck-top{display:flex;align-items:flex-start;gap:12px}
    .home-deck-info{min-width:0;flex:1}
    .home-deck-info h3{margin:0 0 5px;font-size:15px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .home-deck-info p{margin:0;color:#7b8398;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .home-deck-progress{height:7px;background:#eceef5;border-radius:99px;overflow:hidden;margin:18px 0 12px}
    .home-deck-progress span{display:block;height:100%;background:linear-gradient(90deg,#6555e8,#8b7cf4);border-radius:inherit}
    .home-deck-foot{display:flex;align-items:center;justify-content:space-between;color:#7b8398;font-size:12px}
    .home-study{border:0;background:transparent;color:#6657e8;font-weight:800;display:inline-flex;align-items:center;gap:3px;cursor:pointer}
    .home-review{background:#fff;border:1px solid #e3e6ef;border-radius:18px;padding:20px;display:flex;align-items:center;justify-content:space-between;gap:20px}
    .home-review-copy{display:flex;align-items:center;gap:14px}
    .home-review-icon{width:46px;height:46px;border-radius:13px;background:#e9f8f1;color:#19956a;display:grid;place-items:center;flex:0 0 auto}
    .home-review h3{margin:0 0 4px;font-size:16px}
    .home-review p{margin:0;color:#7b8398;font-size:13px}
    .home-empty{background:#fff;border:1px dashed #d8dce8;border-radius:18px;padding:34px;text-align:center;color:#747c91}
    .home-empty h3{margin:0 0 6px;color:#20283a}
    @media(max-width:900px){.home-welcome{align-items:flex-start;flex-direction:column}.home-welcome-action{width:100%}.home-primary{width:100%;justify-content:center}.home-stats{grid-template-columns:1fr}.home-decks{grid-template-columns:1fr}}
  `}</style>;
}
...
function getDeckProgress(deck) {
  ...
  return Math.max(8, Math.min(100, Math.round(((cards.length - due) / cards.length) * 100)));
}
...
function App() {
  ...
  const nav=next=>{const icons={home:Sparkles,library:Library,generate:WandSparkles,stats:TrendingUp};const labels={home:"Início",library:"Meus baralhos",generate:"Gerar com IA",stats:"Progresso"};const Icon=icons[next];return <button className={tab===next?"nav-item active":"nav-item"} onClick={()=>setTab(next)}><Icon size={19}/> {labels[next]}</button>;};
  ...
  <main className="main"><header className="topbar"><div className="mobile-brand"><Brain size={20}/> MemoriaFlash</div><div className="topbar-spacer"></div><button className="profile">{user.displayName?.[0]?.toUpperCase()||"M"}</button></header>
  {tab==="home"&&<div className="page"><HomeStyles/><div className="home-dashboard">
  <section className="home-welcome">
    <div className="home-welcome-copy">
      <span className="eyebrow"><Sparkles size={14}/> CONTA SINCRONIZADA</span>
      <h1>Olá, {user.displayName?.split(" ")[0]||"estudante"}.</h1>
      <p>Continue seus estudos com os mesmos baralhos e cards disponíveis no aplicativo mobile.</p>
    </div>
    <div className="home-welcome-action">
      <button className="home-primary" onClick={()=>setTab("library")}>Ver meus baralhos <ChevronRight size={17}/></button>
    </div>
  </section>

  <section className="home-stats">
    <div className="home-stat"><div className="home-stat-icon"><Library size={21}/></div><div><strong>{decks.length}</strong><span>baralhos sincronizados</span></div></div>
    <div className="home-stat"><div className="home-stat-icon"><BookOpen size={21}/></div><div><strong>{allDeckCards.length.toLocaleString("pt-BR")}</strong><span>cards disponíveis</span></div></div>
    <div className="home-stat"><div className="home-stat-icon"><TrendingUp size={21}/></div><div><strong>{dueCards.length.toLocaleString("pt-BR")}</strong><span>cards para revisar</span></div></div>
  </section>

  <section className="home-section">
    <div className="home-section-head">
      <div><h2>Seus baralhos</h2><p>Continue de onde parou.</p></div>
      {decks.length>4&&<button className="home-link" onClick={()=>setTab("library")}>Ver todos <ChevronRight size={15}/></button>}
    </div>

    {decks.length>0
      ? <div className="home-decks">{decks.slice(0,6).map(deck=><article className="home-deck" key={deck.id}>
          <div className="home-deck-top">
            <div className="subject-icon violet"><BookOpen size={20}/></div>
            <div className="home-deck-info"><h3>{deck.title}</h3><p>{deck.cards.length} cards · {deck.category||"Meu baralho"}</p></div>
          </div>
          <div className="home-deck-progress"><span style={{width:`${getDeckProgress(deck)}%`}} /></div>
          <div className="home-deck-foot"><span>Sincronizado</span><button className="home-study" onClick={()=>startStudy(deck)}>Estudar <ChevronRight size={15}/></button></div>
        </article>)}</div>
      : <div className="home-empty"><h3>Nenhum baralho sincronizado ainda</h3><p>Crie um baralho no aplicativo ou gere seus primeiros flashcards no site.</p><button className="home-primary" onClick={()=>setTab("generate")}><WandSparkles size={17}/> Gerar flashcards</button></div>}
  </section>

  <section className="home-review">
    <div className="home-review-copy">
      <div className="home-review-icon"><TrendingUp size={22}/></div>
      <div><h3>{dueCards.length ? "Você tem cards para revisar" : "Tudo em dia"}</h3><p>{dueCards.length ? `${dueCards.length} cards estão prontos para uma nova revisão.` : "Nenhum card vencido para revisão neste momento."}</p></div>
    </div>
    {dueCards.length>0&&decks.length>0&&<button className="home-primary" onClick={()=>startStudy(decks.find(deck=>deck.cards.some(card=>card.dueDate&&new Date(card.dueDate)<=new Date()))||decks[0])}>Começar revisão <ChevronRight size={17}/></button>}
  </section>
  ...
  {tab==="stats"&&<div className="page progress-dashboard">
    <div className="page-title"><span className="eyebrow"><TrendingUp size={15}/> Progresso</span><h1>Seu desempenho</h1><p>Acompanhe seus baralhos e o que precisa de revisão.</p></div>
    <div className="progress-hero"><div><h2>Continue evoluindo</h2><p>{allDeckCards.length ? `${allDeckCards.length.toLocaleString("pt-BR")} cards sincronizados na sua conta.` : "Ainda não há cards sincronizados."}</p></div><div className="progress-score"><div><strong>{allDeckCards.length ? Math.max(0, Math.round((1-(dueCards.length/allDeckCards.length))*100)) : 0}%</strong><span>em dia</span></div></div></div>
    <div className="progress-metrics">
      <div className="progress-metric"><strong>{decks.length}</strong><span>baralhos</span></div>
      <div className="progress-metric"><strong>{allDeckCards.length}</strong><span>cards sincronizados</span></div>
      <div className="progress-metric"><strong>{dueCards.length}</strong><span>para revisar</span></div>
      <div className="progress-metric"><strong>{subscription.isPro?"PRO":"FREE"}</strong><span>{subscription.isPro?"sem anúncios":"plano atual"}</span></div>
    </div>
    <section className="progress-panel"><h2>Revisão por baralho</h2><p>Cards que estão vencidos para revisão em cada baralho.</p>
      {decks.length ? decks.slice(0,10).map(deck=>{const total=deck.cards?.length||0;const due=(deck.cards||[]).filter(card=>card.dueDate&&new Date(card.dueDate)<=new Date()).length;const pct=total?Math.round((1-due/total)*100):0;return <div className="progress-row" key={deck.id}><div className="progress-row-head"><span>{deck.title}</span><span>{pct}% em dia</span></div><div className="progress-track"><div style={{width:`${pct}%`}}/></div></div>}) : <div className="empty-state"><h3>Nenhum baralho sincronizado</h3><p>Seus baralhos aparecerão aqui quando estiverem disponíveis no Firebase.</p></div>}
    </section>
    <AdSlot isPro={subscription.isPro} slot={import.meta.env.VITE_ADSENSE_STATS_SLOT}/>
  </div>}
  </main></div>;
}
