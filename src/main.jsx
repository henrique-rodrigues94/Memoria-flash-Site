...
import {
  ...
  Search, Sparkles, TrendingUp, ThumbsDown, ThumbsUp, Crown,
  WandSparkles, Clock3, RotateCcw,
} from "lucide-react";
import {
  GoogleAuthProvider,
  getIdToken,
...
const GRADES = { hard: "hard", good: "good", easy: "easy" };
...
function reviewCard(card, grade) {
  ...
  return {...card, reps, interval, efactor: nextEf, dueDate: new Date(now.getTime() + interval * 86400000).toISOString(), lastReviewed: now.toISOString()};
}
...
function App() {
  ...
  useEffect(() => {
    if (!user) { setDecks([]); return undefined; }
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, "userStats", user.uid), snap => {
      ...
      const expired = expiryDate && !Number.isNaN(new Date(expiryDate).getTime()) && new Date(expiryDate).getTime() <= Date.now();
  ...
  const filteredDecks = useMemo(() => { const q=query.trim().toLowerCase(); if(!q) return decks; return decks.filter(d => `${d.title} ${d.category} ${d.description}`.toLowerCase().includes(q)); }, [decks, query]);
  ...
  async function login() {
    ...
    try { await authPersistenceReady; const provider=new GoogleAuthProvider(); provider.setCustomParameters({prompt:"select_account"}); await signInWithPopup(auth, provider); }
    catch(error) {
      const code=error?.code || "";
      if(code.includes("popup-blocked") || code.includes("popup-failed") || code.includes("operation-not-supported-in-this-environment")) {
        try { const provider=new GoogleAuthProvider(); provider.setCustomParameters({prompt:"select_account"}); await signInWithRedirect(auth, provider); return; } catch(redirectError) { setAuthError(authErrorMessage(redirectError)); }
  ...
  function openDeck(deck) { setSelectedDeck(deck); setStudyCards(deck.cards || []); setStudyIndex(0); setRevealed(false); setFeedback(null); setFeedbackReason(""); setFeedbackComment(""); setFeedbackStatus(""); setTab("study"); }

  async function openOfficialCards(subject, topic, level) {
    setStudyLoading(true);
    try { const cards=await getCards({subject, topic, level, cardType:"definition"}); setStudyCards(cards); setSelectedDeck(null); setStudyIndex(0); setRevealed(false); setTab("study"); }
  ...
  async function gradeCard(grade) {
    ...
    await saveCardProgress(user.uid, card.id, {reviewed:true, grade, deckId:card.deckId || selectedDeck?.id || null, ...updated});
    setStudyIndex(i=>i+1); setRevealed(false); setFeedback(null); setFeedbackReason(""); setFeedbackComment(""); setFeedbackStatus("");
  }

  async function sendFeedback(rating) {
    if(!card) return; setFeedback(rating);
  ...
  async function generateCards() {
    ...
    setGenerateBusy(true); setGenerateError(""); setGenerated([]);
    try {
      const token=await getIdToken(auth.currentUser);
      const base=(import.meta.env.VITE_API_URL || import.meta.env.VITE_BACKEND_URL || "").replace(/\/$/, "");
      const response=await fetch(`${base}/api/gemini/generate-flashcards`, {method:"POST", headers:{"Content-Type":"application/json", Authorization:`Bearer ${token}`}, body:JSON.stringify({prompt:`Gere flashcards para a matéria "${generateForm.subject.trim()}" focando exclusivamente no tópico "${generateForm.topic.trim()}".`, subject:generateForm.subject.trim(), topic:generateForm.topic.trim(), selectedTopics:[generateForm.topic.trim()], educationLevel:generateForm.level, count:Math.min(50,Math.max(1,Number(generateForm.count)||10)), difficulty:generateForm.difficulty, language:"pt", cardContentType:"definition", sourceType:"subject", existingFronts:allDeckCards.map(c=>c.front).filter(Boolean).slice(0,500)})});
      ...
      setGenerated(cards); setTab("generate");
    } catch(error) { setGenerateError(error?.name === "AbortError" ? "A geração demorou demais." : error?.message || "Não foi possível gerar os cards."); }
  ...
  const nav=(next)=><button className={tab===next ? "nav-item active":"nav-item"} onClick={()=>setTab(next)}>{next==="home"?<Sparkles size={19}/>:next==="library"?<Library size={19}/>:next==="study"?<BookOpen size={19}/>:next==="generate"?<WandSparkles size={19}/>:<TrendingUp size={19}/>} {next==="home"?"Início":next==="library"?"Meus baralhos":next==="study"?"Estudar":next==="generate"?"Gerar com IA":"Progresso"}</button>;
