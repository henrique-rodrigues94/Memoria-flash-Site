...
function normalizeDeck(snapshot) {
  ...
  const deck = {
    ...
    cards: Array.isArray(data.cards)
      ? data.cards.map((card) => normalizeCard(card, { id: snapshot.id, ...data }))
      : [],
  };
...
/**
 * Escuta os mesmos documentos `decks` utilizados pelo aplicativo mobile.
 * O backend/app usa userId = uid para decks particulares e `public`/`system`
