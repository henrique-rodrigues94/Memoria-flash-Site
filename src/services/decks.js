...
function normalizeDeck(snapshot) {
  ...
  return {
    ...
    cards: Array.isArray(data.cards) ? data.cards.map((card) => normalizeCard(card, deck)) : [],
  };
}
...
export function subscribeToUserDecks(userId, onUpdate, onError) {
  ...
  return onSnapshot(q, (snapshot) => {
    const decks = snapshot.docs.map(normalizeDeck).sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""));
    onUpdate(decks);
  }, (error) => {
...
export function flattenDeckCards(decks) {
  return decks.flatMap((deck) => deck.cards.map((card) => ({ ...card, deckId: deck.id, deckTitle: deck.title })));
}

export async function createUserDeck({ userId, title, category, description = "", cards = [] }) {
